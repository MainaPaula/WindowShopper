...
class _BizRegistrationState extends State<BizRegistration> {
    ...
    setState(() {
      ...
      businesses.where('users', isEqualTo: {currentUser})
        ...
        if (querySnapshot.docs.isNotEmpty) {
          ...
          businesses.add({'business owner': {currentUser: null}
          }).then((value) =>
          {
  ...
  /*Future pickLogo() async {
    final img = await image.pickImage(source: ImageSource.gallery, imageQuality: 50);
    setState(() {
      _file = File(img!.path);
      bizLogo = _file!.name;
    });
  ...
  uploadFile() async {
    try {
      var imagefile =
      FirebaseStorage.instance.ref().child("Users").child("/$bizLogo");
